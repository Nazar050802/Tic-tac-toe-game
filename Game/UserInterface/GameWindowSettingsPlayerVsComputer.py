...
class GameWindowSettingsPlayerVsComputer(AdditionalWindow):
    def __init__(self, parent_window=None):
        super().__init__(window_size, window_name, parent_window)

        self.set_window_background_image("TkinterStyle/images/menu_player_vs_computer_background.png",
                                         [0, 0], [-3, 1])
        ...
        self._combobox_with_computer_difficulty = None
    ...
    def create_setting_number_of_players(self, current_number_of_players):
        ...
        current_number_of_players.set(number_of_players["min_amount"] - 1)
        ...
        scale_with_number_of_players = ttk.Scale(master=frame_number_of_players,
                                                 from_=number_of_players["min_amount"] - 1,
                                                 to=number_of_players["max_amount"] - 1,
                                                 orient="horizontal",
    ...
    def create_choose_difficulty_of_computer(self):
        computer_difficulties = CommunicationWithConfig.get_from_config_computer_difficulties()
        computer_difficulties = [i.capitalize() for i in computer_difficulties]

        # Create LabelFrame
        labelframe_computer_difficulties = ttk.LabelFrame(self, text="SELECT COMPUTER DIFFICULTY")
        labelframe_computer_difficulties.grid(row=2, ipadx=3, pady=10)

        # Create frame inside LabelFrame
        frame_symbol_of_player = tk.Frame(master=labelframe_computer_difficulties)
        frame_symbol_of_player.grid(row=0, pady=6, padx=(3, 0))

        combobox_player_symbols = ttk.Combobox(master=frame_symbol_of_player,
                                               values=computer_difficulties,
                                               state="readonly")
        combobox_player_symbols.current(0)
        combobox_player_symbols.pack(padx=(18, 0), pady=6)

        return combobox_player_symbols

    def create_next_step_button(self):
        frame = tk.Frame(master=self)
        frame.grid(row=3, column=0, padx=14)
    ...
    def create_return_button(self):
        frame = tk.Frame(master=self)
        frame.grid(row=4, column=0, padx=14, pady=(14, 6))
    ...
    def create_game_settings(self):
        ...
        self._combobox_with_computer_difficulty = self.create_choose_difficulty_of_computer()
    ...
    def next_step_window(self):
        ...
        computer_difficulty = self._combobox_with_computer_difficulty.get()
        computer_difficulty_in_int = -1

        if computer_difficulty == "Easy":
            computer_difficulty_in_int = 0
        elif computer_difficulty == "Normal":
            computer_difficulty_in_int = 1
        elif computer_difficulty == "Hard":
            computer_difficulty_in_int = 2

        game_window = WindowPlayerInformation(self, self._number_of_players.get(), self._board_size.get(),
                                              computer_difficulty_in_int)
